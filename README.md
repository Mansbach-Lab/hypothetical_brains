## hypotheticalbrains
[![Build Status](https://travis-ci.org/uwescience/hypotheticalbrains.svg?branch=master)](https://travis-ci.org/uwescience/hypotheticalbrains)
...
`hypotheticalbrains/hypotheticalbrains.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `hypotheticalbrains.py` in directory called
`hypotheticalbrains`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import hypotheticalbrains as sb` in an
interactive Python session, the classes and functions defined inside of the
`hypotheticalbrains.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the hypotheticalbrains module, the main classes defined at the bottom of
...
Either way, you can create a `hypotheticalbrains/data` folder in which you can
...
the data at:
    ...
    import hypotheticalbrains as sb
...
(typically, something like our `hypotheticalbrains/tests/test_hypotheticalbrains.py`). Within each
...
have one such test in `hypotheticalbrains/tests/test_hypotheticalbrains.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/hypotheticalbrains`),
and type:

    py.test hypotheticalbrains

This will exercise all of the tests in your code directory. If a test fails, you
will see a message such as:


    hypotheticalbrains/tests/test_hypotheticalbrains.py .F...
    ...
    hypotheticalbrains/tests/test_hypotheticalbrains.py:49: AssertionError
...
To document `hypotheticalbrains` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/hypotheticalbrains/blob/master/doc/_static/RTD-advanced-conf.png)

 http://hypotheticalbrains.readthedocs.org/en/latest/
...
A `hypotheticalbrains/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/hypotheticalbrains) for the software. This
...
`hypotheticalbrains` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `hypotheticalbrains`, we use the
...
[here](https://github.com/uwescience/hypotheticalbrains/blob/master/scripts/Figure1.ipynb).
...
### Using `hypotheticalbrains` as a template
...
button on the front page of the "hypotheticalbrains" repository.
...
`hypotheticalbrains/hypotheticalbrains.py` to be called `smallish/smallish.py`

	git mv hypotheticalbrains smallish
	git mv smallish/hypotheticalbrains.py smallish/smallish.py
	git mv smallish/tests/test_hypotheticalbrains.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `hypotheticalbrains` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `hypotheticalbrains` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `hypotheticalbrains` in them. Type
the following to see where all these files are:

	git grep hypotheticalbrains

You can replace `hypotheticalbrains` for `smallish` quickly with:

	git grep -l 'hypotheticalbrains' | xargs sed -i 's/hypotheticalbrains/smallish/g'
...
Edit all the mentions of `hypotheticalbrains` in the `hypotheticalbrains/__init__.py` file, and in
the `hypotheticalbrains/version.py` file as well.
